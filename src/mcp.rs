...
fn tool_definitions() -> Vec<Value> {
    vec![
        ...
        tool(
            "portal_start",
            "Start a Wayland RemoteDesktop/ScreenCast portal session. This may show a system permission prompt; saved restore tokens are reused only when the compositor supports persistent RemoteDesktop grants.",
            json!({
                "type": "object",
                "properties": {}
            }),
